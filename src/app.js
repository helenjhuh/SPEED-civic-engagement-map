...
/* Test filter map */
app.get("/filter", (req, res) => res.render("pages/filter"));
...
app.get("/cem_map", (req, res) => {
  ...
  geoUser.find({}, (err, pinGeo) => {
    err
      ...
      : res.render("pages/index", { users: JSON.stringify(pinGeo) });
  });
});
        ...
        if (err) {
          ...
          for (let i = 0; i < thisUser.usersPins.length; i++) {
            ...
            if (thisUser.usersPins[i].equals(req.params.id)) {
              ...
              thisUser.usersPins = thisUser.usersPins
                .slice(0, i)
                .concat(thisUser.usersPins.slice(-i));
            }
          }
